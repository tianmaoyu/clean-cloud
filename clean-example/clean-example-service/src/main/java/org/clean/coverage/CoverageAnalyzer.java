package org.clean.coverage;// 引入必要的包
...
//            AuthorInfo classAuthor = getClassAuthorInfo(classNode);
        ...
        System.out.printf("已覆盖方法数: %d (%.1f%%)\n", coveredMethods,
                totalMethods > 0 ? (double) coveredMethods / totalMethods * 100 : 0);
        System.out.printf("总圈复杂度: %d\n", totalComplexity);
        System.out.printf("已覆盖圈复杂度: %d (%.1f%%)\n", coveredComplexity,
                totalComplexity > 0 ? (double) coveredComplexity / totalComplexity * 100 : 0);
        ...
        for (Map.Entry<String, PackageStats> entry : packageStats.entrySet()) {
            ...
            double methodCoverage = stats.totalMethods > 0 ?
                    (double) stats.coveredMethods / stats.totalMethods * 100 : 0;
            double complexityCoverage = stats.totalComplexity > 0 ?
                    (double) stats.coveredComplexity / stats.totalComplexity * 100 : 0;
    ...
    // 生成 HTML 报告
    public void generateHtmlReport(String sourceDir, String classDir,
                                   String execFile, String reportDir) throws IOException {
        // 创建执行数据存储
        ExecutionDataStore executionData = new ExecutionDataStore();
        SessionInfoStore sessionInfos = new SessionInfoStore();
        ...
        visitor.visitBundle(coverageBuilder.getBundle("Project"),
                new DirectorySourceFileLocator(
                        new File(sourceDir), "utf-8", 4));
        visitor.visitEnd();
    ...
    public static void main(String[] args) {
        ...
        try {
            // 示例用法
            String basePath = "/Users/eric/clean-cloud/clean-system/clean-system-service";
